...
BJH050_data <- read_csv(path(here(), './data/ieeg_behave/BJH050_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# clean Biscuits #
BJH050_data_clean <- clean_bci_data(BJH050_data, 2000)

# create distance df #
BJH050_data_distance <- create_distance_df_bci(BJH050_data_clean)

# task vars df #
BJH050_all_vars_df <- get_across_task_variables_bci(BJH050_data_clean)
...
no_biscuits_trials <- BJH050_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
length(unique(BJH050_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/BJH050_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/BJH050_bad_trials.csv'))

create_last_away_events(BJH050_data_clean, 'BJH050_last_away_events.csv')
create_first_dot_events(BJH050_data_clean, 'BJH050_first_dot_events.csv')
create_first_move_events(BJH050_data_clean, 'BJH050_first_move_events.csv')
create_died_events(BJH050_data_clean, 'BJH050_died_events.csv')
create_attack_events(BJH050_data_clean, 'BJH050_attack_events.csv')
