...
BJH041_data <- read_csv(path(here(), './data/ieeg_behave/BJH041_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# clean Biscuits #
BJH041_data_clean <- clean_bci_data(BJH041_data, 2000)

# create distance df #
BJH041_data_distance <- create_distance_df_bci(BJH041_data_clean)

# task vars df #
BJH041_all_vars_df <- get_across_task_variables_bci(BJH041_data_clean)
...
no_biscuits_trials <- BJH041_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
# long_trials <- BJH041_all_vars_df %>%
...
length(unique(BJH041_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/BJH041_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/BJH041_bad_trials.csv'))

create_last_away_events(BJH041_data_clean, 'BJH041_last_away_events.csv')
create_first_dot_events(BJH041_data_clean, 'BJH041_first_dot_events.csv')
create_first_move_events(BJH041_data_clean, 'BJH041_first_move_events.csv')
create_died_events(BJH041_data_clean, 'BJH041_died_events.csv')
create_attack_events(BJH041_data_clean, 'BJH041_attack_events.csv')
