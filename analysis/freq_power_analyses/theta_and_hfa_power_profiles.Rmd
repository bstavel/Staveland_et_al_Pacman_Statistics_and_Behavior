...
## Local Power Profiles

This script creates figures for average theta and HFA power profiles. 

Inputs:
* `theta_ieeg_REGION_all_subs_logged_iti_onset.csv` from  `analysis/freq_power_analyses/compile_ieeg_files_iti_logged_trialonset.Rmd`
...
mfg_subject_supplement_plot <- mfg_example_turn_sub_avg_theta_df %>%
  ...
  geom_line(size =.5) +
  geom_ribbon(aes(ymin = lower_sem, ymax = upper_sem), alpha = 0.2) +
  scale_color_manual(values = c("#876194", "darkgrey")) +
  scale_fill_manual(values = c("#876194", "darkgrey")) +
  theme(panel.background = element_rect(fill = "white"),
      legend.position = "top",
      axis.text = element_text(family = "Gill Sans", color = "#2D2327", size = 7),
      axis.title = element_text(family = "Gill Sans", color = "#2D2327", size = 10),
