...
SLCH018_data <- read_csv(path(here(), './data/ieeg_behave/SLCH018_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# clean Biscuits #
SLCH018_data_clean <- clean_bci_data(SLCH018_data, 2000)

# create distance df #
SLCH018_data_distance <- create_distance_df_bci(SLCH018_data_clean)

# task vars df #
SLCH018_all_vars_df <- get_across_task_variables_bci(SLCH018_data_clean)
...
no_biscuits_trials <- SLCH018_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
long_trials <- SLCH018_all_vars_df %>%
...
length(unique(SLCH018_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/SLCH018_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/SLCH018_bad_trials.csv'))

create_last_away_events(SLCH018_data_clean, 'SLCH018_last_away_events.csv')
create_first_dot_events(SLCH018_data_clean, 'SLCH018_first_dot_events.csv')
create_first_move_events(SLCH018_data_clean, 'SLCH018_first_move_events.csv')
create_died_events(SLCH018_data_clean, 'SLCH018_died_events.csv')
create_attack_events(SLCH018_data_clean, 'SLCH018_attack_events.csv')
...
trial_times <- SLCH018_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
