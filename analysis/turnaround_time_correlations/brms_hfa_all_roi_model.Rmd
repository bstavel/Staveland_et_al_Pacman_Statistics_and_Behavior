...
## HFA Synchrony ~ Approach Times

This script tests the hypothesis that approach times and the degree of HFA synchrony on a given trial are correlated. This is done with a linear mixed effects model using an interaction between Approach Time and Region to predict synchrony. This is run onlhy within the electrodes that showed elevated HFA coherence, though we rerun the models on multiple different HFA thresholds.

Inputs:
* `hfa_correlations_newsubs.csv` which is calculated in `calculate_trial_correlations.Rmd`
* `turnaround_correl_trial_turn_times.csv` which is calculated in `calculate_trial_correlations.Rmd`
...
* `sig_theta_pairs.csv` which is calculated in `create_sig_theta_coherence_csv.Rmd`

Outputs:
* `full_all_roi_model_brms_newsubs_noinsula.RData` in `results/` folder (100ms, original)
* `full_HFA_all_roi_all_elecs_model_brms.RData` in `results/` folder (no thresholding)
* `full_HFA_all_roi_10a_elecs_model_brms.RData` in `results/` folder (100ms, approach only)
* `full_HFA_all_roi_50_elecs_model_brms.RData` in `results/` folder (500ms)
* `full_HFA_all_roi_100_elecs_model_brms.RData` in `results/` folder (1000ms
* `HFA_all_roi_orig_elecs_slopes_summary.csv` in `results` folder
* `HFA_all_roi_all_elecs_slopes_summary.csv` in `results` folder
* `HFA_all_roi_10a_elecs_slopes_summary.csv` in `results` folder
* `HFA_all_roi_50_elecs_slopes_summary.csv` in `results` folder
* `HFA_all_roi_100_elecs_slopes_summary.csv` in `results` folder

Note that some of these were actually calculated in `high_performance_cluster_batch_scripts` for faster computation times
...
# load full model #
load(path(here(), "results", "full_hfa_all_roi_50_elecs_model_brms.RData"))
hfa_50_elec_model <- hfa_100_elec_model ## originally messed up naming!!
...
# plot #
mcmc_trace(hfa_50_elec_model, regex_pars = c("b_"), facet_args = list(ncol = 1))
...
# Combine posteriors into CIs for each region 
slopes_summary_df <- get_region_effects_for_turnaround_models(hfa_50_elec_model,unique(correlation_clean_df$region_pair))

# save output
write_csv(slopes_summary_df, path(here(), "results", "turn_time_prediction", "hfa_all_roi_50_elecs_slopes_summary.csv"))
...
# Combine posteriors into CIs for each region 
slopes_summary_df <- get_region_effects_for_turnaround_models(hfa_100_elec_model,unique(hfa_100_elec_model$data$region_pair))
...
# load full model #
load(path(here(), "results",  "full_hfa_all_roi_10a_elecs_model_brms.RData"))
hfa_10a_elec_model <- model
...
# plot #
mcmc_trace(hfa_10a_elec_model, regex_pars = c("b_"), facet_args = list(ncol = 1))
...
# Combine posteriors into CIs for each region 
slopes_summary_df <- get_region_effects_for_turnaround_models(hfa_10a_elec_model,unique(hfa_10a_elec_model$data$region_pair))

# save output
write_csv(slopes_summary_df, path(here(), "results", "turn_time_prediction", "hfa_all_roi_10a_elecs_slopes_summary.csv"))
