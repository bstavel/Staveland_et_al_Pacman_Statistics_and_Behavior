...
## Some EDA and alternate behavior models in the task, including the sex*turning distance model

Mostly, this was not used in the paper, except for the sex*turning distance interaction model. 
...
```{r bind-demo-turn-time}

# add demo data
game_data_distance <- left_join(game_data_distance, combined_demo %>% select(ID, Age, Sex), by = c("ID"))
...
clean_data_distance <- game_data_distance %>%
  select(subject, trial_numeric, trial_flip, dots_eaten, away_choice, last_away, attack_chase_bob, Score, Lives, died, Sex, distance_to_ghost, reward_groups, ghost_start_dir, case) %>%
  distinct() %>%
...
turn_reward_df <- game_data_distance %>%
  select(reward_groups, last_away, trial_numeric, subject, case, Sex) %>%
  filter(last_away != 0) %>%
...
# Fit the model
behave_model <- brm(
  formula = last_away ~ large_reward*Sex + (1 + large_reward | subject),
  data = turn_reward_df,
  prior = priors,
  family = gaussian(),
  iter = 5000,
...
posterior_samples <- posterior_samples(behave_model)
prob_mu_gt_0 <- mean(behave_model$b_SexMale > 0)


# save full model #
save(behave_model, file = path(here(), "results", "all_subs_sex_turn_reward_model_brms.RData"))
...
# Fit the model
behavior_model <- brm(
  formula = last_away ~ last_reward + ghost_start_dir + initial_score + Lives + anxiety_score + Sex +
