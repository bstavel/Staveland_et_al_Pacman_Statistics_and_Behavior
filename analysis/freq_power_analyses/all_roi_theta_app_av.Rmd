...
This script tests for elevated theta power during approach in the hippocampus, amygdala, OFC, ACC, & MFG,in both conflict and no-conflict trials. The window of interest is from 1.5 seconds before turning until 1.5 seconds after turning. I average this window within trials, electrodes, and subjects and then compare the approach vs avoiding sections using the bayesian mixed effects modeling. This script also creates the figures associated with this analysis in Figure 2

Inputs: 
* `theta_ieeg_REGION_all_subs_logged_iti_onset.csv` which are outputs of `analyhsis/freq_power_analyses/compile_ieeg_files_iti_logged_trialonset.Rmd`
* `all_subs_complete_distance_df.csv` which is an output of `analysis/cleaning_ieeg_behavior/combine_ieeg_subs_behavior.Rmd`

Outputs:
* `app_avd_REGION_theta_model_noattack_brms.RData` in the `results`/` folder
* `app_avd_REGION_conflictfree_theta_model_brms.RData` in the `results/` folder
* `figure2_REGION_app_avoid_combined_theta_plot.png` in the `figures/` folder
