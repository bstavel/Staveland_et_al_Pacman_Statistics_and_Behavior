...
## Significant Theta Coherence Electrode Pairs

The main point of this script is to create a csv with the significant electrode pairs based on the theta coherence analysis. Additionally, it creates some supporitng figures and tables about the number of significant electrodes.
...
It produces: 
* `sig_theta_pairs.csv` in the `results/` folder
* `sig_theta_threshold_pairs.csv` in the `results/` folder (alternate thresholds used in the supplementary analyses)
* `figure3_theta_coherence_across_metrics.png` in the `figures/connectivity` folder
...
conn_summary_plot <- conn_summary_df %>%
  ...
  scale_fill_manual(values = c("#F89078", "#EA4E87", "#A2319F")) +
