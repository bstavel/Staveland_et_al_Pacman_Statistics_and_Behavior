...
## Theta Synchrony ~ Approach Times

This script tests the hypothesis that approach times and the degree of theta synchrony on a given trial are correlated. This is done with a linear mixed effects model using an interaction between Approach Time and Region to predict synchrony. This is run onlhy within the electrodes that showed elevated theta coherence, though we rerun the models on multiple different theta thresholds.

Inputs:
* `theta_correlations_newsubs.csv` which is calculated in `calculate_trial_correlations.Rmd`
...
* `full_theta_all_roi_all_elecs_model_brms.RData` in `results/` folder (no thresholding)
* `full_theta_all_roi_10a_elecs_model_brms.RData` in `results/` folder (100ms, approach only)
* `full_theta_all_roi_50_elecs_model_brms.RData` in `results/` folder (500ms)
* `full_theta_all_roi_100_elecs_model_brms.RData` in `results/` folder (1000ms
* `theta_all_roi_orig_elecs_slopes_summary.csv` in `results` folder
* `theta_all_roi_all_elecs_slopes_summary.csv` in `results` folder
* `theta_all_roi_10a_elecs_slopes_summary.csv` in `results` folder
* `theta_all_roi_50_elecs_slopes_summary.csv` in `results` folder
* `theta_all_roi_100_elecs_slopes_summary.csv` in `results` folder
...
turn_time_df <- read_csv(path(here(), "munge", "turnaround_correl_trial_turn_times.csv"))


# join #
correlation_df <- left_join(correlation_df, turn_time_df)
