...
BJH017_data <- read_csv(path(here(), './data/ieeg_behave/BJH017_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
BJH017_data <- BJH017_data %>%
  mutate(Time = Sample) # old conversion script had different variables

# clean Biscuits #
BJH017_data_clean <- clean_bci_data(BJH017_data, 2000)

# create distance df #
BJH017_data_distance <- create_distance_df_bci(BJH017_data_clean)

# task vars df #
BJH017_all_vars_df <- get_across_task_variables_bci(BJH017_data_clean)
...
no_biscuits_trials <- BJH017_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
long_trials <- BJH017_all_vars_df %>%
...
BJH017_data_clean %>%
  ...
  ylim(0, 200) +
  theme(panel.background = element_rect(fill = "white")) +
  facet_wrap(~trial_numeric)# no long trials
...
length(unique(BJH017_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/BJH017_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/BJH017_bad_trials.csv'))

create_last_away_events(BJH017_data_clean, 'BJH017_last_away_events.csv')
create_first_dot_events(BJH017_data_clean, 'BJH017_first_dot_events.csv')
create_first_move_events(BJH017_data_clean, 'BJH017_first_move_events.csv')
create_died_events(BJH017_data_clean, 'BJH017_died_events.csv')
create_attack_events(BJH017_data_clean, 'BJH017_attack_events.csv')
