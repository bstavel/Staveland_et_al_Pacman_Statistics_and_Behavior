...
## Directionality Estimation for Cross-Correlation Analysis

This script takes the correlation at the best lag from the true cross-correlation analysis and compares them to the correlations created from shuffling the data over trials. It then moves forward all significant electrode pairs (p < 0.05) to directionality estimation using the sign of the lag at which the best correlation occurred. Finally, it calculate the P+ for each region pair and concatenates them in a table.
...
ccf_ess['OFC_Cing'] <- round(summary(sub_ofc_cing_model)$fixed[1, 6], 1)
...
ccf_ess['OFC_HC'] <- round(summary(sub_ofc_hc_model)$fixed[1, 6], 1)
...
ofc_hc_df <- tibble(
  "Region Pair" = "OFC_HC",
  "Prob Pos." = ccf_prob_ps["OFC_HC"],
  create_row_in_cff_table(ccf_ofc_hc_results, ccf_ofc_hc_results_sig))

ofc_cing_df <- tibble(
  "Region Pair" = "OFC_Cing",
  "Prob Pos." = ccf_prob_ps["OFC_Cing"],
  create_row_in_cff_table(ccf_ofc_cing_results, ccf_ofc_cing_results_sig))
