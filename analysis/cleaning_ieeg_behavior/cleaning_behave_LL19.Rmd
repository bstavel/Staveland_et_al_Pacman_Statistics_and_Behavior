...
LL19_data <- read_csv(path(here(), './data/ieeg_behave/LL19_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# task vars df #
LL19_all_vars_df <- get_across_task_variables_bci(LL19_data_clean)
...
no_biscuits_trials <- LL19_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
length(unique(LL19_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/LL19_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/LL19_bad_trials.csv'))

create_last_away_events(LL19_data_clean, 'LL19_last_away_events.csv')
create_first_dot_events(LL19_data_clean, 'LL19_first_dot_events.csv')
create_first_move_events(LL19_data_clean, 'LL19_first_move_events.csv')
create_died_events(LL19_data_clean, 'LL19_died_events.csv')
create_attack_events(LL19_data_clean, 'LL19_attack_events.csv')
