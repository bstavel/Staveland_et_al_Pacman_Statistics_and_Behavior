...
## Cross Correlation Analysis for estimating Directionality

This script takes bandpass estimates of theta for each electrode pair with elevated theta coherence and runs a cross-correlation analysis to identify the best lag. It also create a null distribution from shuffling the trial data within each pair 200 times. Data are saved out and carried forward to `theta_ccf_stats.Rmd`.

Inputs: 
* csvs in the  `bandpass_theta` folder with bandpassed theta data for each subject (created in `Staveland_et_al_Pacman_Neural_Analyses/blob/main/preprocessing/scripts/bandpass_theta_calculation.ipynb`)

Outputs:
* Both the True (`ccf_REGION1_REGION2_results.csv`) and Null (`null_ccf_REGION1_REGION2_results.csv`)  CSVs


```{r load-theta-files, eval = T}

## only run once, saved version is below
...
```{r merge-behave-ieeg-data, eval = T}
...
# read_csv
all_subs_theta_data <- read_csv(path(here(), "munge", "all_regions_bandpassed_theta_iti_onset.csv"))
...
need_to_run_sig_pairs <- read_csv(path(here(), "results", "missing_cc_pairs.csv"))
all_sig_pairs <- need_to_run_sig_pairs
...
ccf_amyg_acc_null_results <- run_and_save_perms(amyg_acc_df, amyg_cing_sig_pairs, all_subs_theta_data, "null_ccf_amyg_acc_results.csv")
...
ccf_hc_amyg_null_results <- run_and_save_perms(hc_amyg_df, sig_pairs, all_subs_theta_data,  "null_ccf_hc_amyg_results.csv")
...
ccf_hc_cing_null_results <- run_and_save_perms(hc_cing_df, sig_pairs, all_subs_theta_data,  "null_ccf_hc_cing_results.csv")
...
ccf_ofc_amyg_null_results <- run_and_save_perms(ofc_amyg_df, sig_pairs, all_subs_theta_data, "null_ccf_ofc_amyg_results.csv")
...
ccf_ofc_acc_null_results <- run_and_save_perms(ofc_acc_df, 
                                               sig_pairs, 
                                               all_subs_theta_data, "null_ccf_ofc_acc_results.csv")
...
ccf_ofc_hc_null_results <- run_and_save_perms(ofc_hc_df, sig_pairs, all_subs_theta_data, "null_ccf_ofc_hc_results.csv")
...
ccf_ofc_mfg_null_results <- run_and_save_perms(ofc_mfg_df, sig_pairs, all_subs_theta_data,  "null_ccf_ofc_mfg_results.csv")
...
ccf_mfg_cing_null_results <- run_and_save_perms(mfg_cing_df, sig_pairs, all_subs_theta_data,  "null_ccf_mfg_cing_results.csv")
...
ccf_mfg_hc_null_results <- run_and_save_perms(mfg_hc_df, sig_pairs, all_subs_theta_data,  "null_ccf_mfg_hc_results.csv")
...
ccf_mfg_amyg_null_results <- run_and_save_perms(mfg_amyg_df, sig_pairs, all_subs_theta_data,  "null_ccf_mfg_amyg_results.csv")
