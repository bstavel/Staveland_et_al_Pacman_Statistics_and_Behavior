...
BJH039_data <- read_csv(path(here(), './data/ieeg_behave/BJH039_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# clean Biscuits #
BJH039_data_clean <- clean_bci_data(BJH039_data, 2000)

# create distance df #
BJH039_data_distance <- create_distance_df_bci(BJH039_data_clean)

# task vars df #
BJH039_all_vars_df <- get_across_task_variables_bci(BJH039_data_clean)
...
no_biscuits_trials <- BJH039_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
# was worried there was trial misnaming because there are too many trials! But it looks lke it was just manually stopped. double checked with all the trial types add up # 210 for every block
# BJH039_all_vars_df %>% group_by(block) %>% mutate(ttype_sum = sum(TrialType))
```
...
long_trials <- BJH039_all_vars_df %>%
...
BJH039_data_clean %>%
  ...
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
  filter(neural_trial_numeric %in% c(11, long_trials)) %>%
  pivot_longer(cols = c(GhostLocation, UserLocation), values_to = "location", names_to = "unit") %>%
  ggplot(., aes(x = trial_flip, y = location, color = unit)) +
  geom_hline(yintercept = 170, color = "black") +
  geom_hline(yintercept = 10, color = "black") +
  ...
  ylim(0, 200) +
  theme(panel.background = element_rect(fill = "white")) +
  facet_wrap(~trial_numeric, scales = "free_x")
...
bad_trials <- c(no_biscuits_trials, long_trials, 4) # manually added later because it looks like it was paused at the end


```


## Check all other trials

```{r all-other-trials, fig.width = 20, fig.height = 30}
...
length(unique(BJH039_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/BJH039_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/BJH039_bad_trials.csv'))

create_last_away_events(BJH039_data_clean, 'BJH039_last_away_events.csv')
create_first_dot_events(BJH039_data_clean, 'BJH039_first_dot_events.csv')
create_first_move_events(BJH039_data_clean, 'BJH039_first_move_events.csv')
create_died_events(BJH039_data_clean, 'BJH039_died_events.csv')
create_attack_events(BJH039_data_clean, 'BJH039_attack_events.csv')
