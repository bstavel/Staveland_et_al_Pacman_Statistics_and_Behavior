...
BJH026_data <- read_csv(path(here(), './data/ieeg_behave/BJH026_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# clean Biscuits #
BJH026_data_clean <- clean_bci_data(BJH026_data, 2000)

# create distance df #
BJH026_data_distance <- create_distance_df_bci(BJH026_data_clean)

# task vars df #
BJH026_all_vars_df <- get_across_task_variables_bci(BJH026_data_clean)

```

In block 6 they tried out only get 1 dot each trial!
...
no_biscuits_trials <- BJH026_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
long_trials <- BJH026_all_vars_df %>%
...
length(unique(BJH026_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/BJH026_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/BJH026_bad_trials.csv'))

create_last_away_events(BJH026_data_clean, 'BJH026_last_away_events.csv')
create_first_dot_events(BJH026_data_clean, 'BJH026_first_dot_events.csv')
create_first_move_events(BJH026_data_clean, 'BJH026_first_move_events.csv')
create_died_events(BJH026_data_clean, 'BJH026_died_events.csv')
create_attack_events(BJH026_data_clean, 'BJH026_attack_events.csv')
