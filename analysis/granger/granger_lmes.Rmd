...
## Directionality Tests on Net Granger Values

This script takes the results from the granger causality analyses and runs linear mixed effects models to determine if the electrode pairs with signifigant directionality are consistently in one direction. It does this across multiple thresholds of directionality to ensure that it is not biased the cohort used.

Inputs:
* `all_subs_all_roi_true_granger_dir_combined.csv` which is a csv combined by running `Staveland_et_al_Pacman_Neural_Analyses/blob/main/connectivity/scripts/granger/compute_granger_pvalues_additional_perms.py` and then combined with an earlier version of the analysis which only had 200 permutations

Outputs:
* `granger_approach_REGION1_REGION2.RData` (500ms of signifigant directionality, the threshold used in the paper)
* `granger_100ms_REGION1_REGION2.RData` (100ms of signifigant directionality)
* `granger_100ams_REGION1_REGION2.RData` (100ms of signifigant directionality, in the apporach period only)
* `granger_1000ms_approach_REGION1_REGION2.RData` (1000ms of signifigant directionality)
* Supplemental Table 7 & 8

```{r load-data}

## Load Granger Data
granger_df <- read_csv(path(here(), "results", "granger",
                                             "all_subs_all_roi_true_granger_dir_combined.csv"))
...
cat("P(Overall mean > 0):", prob_mu_gt_0, "\n")
#save model
save(ofc_mfg_app_model, file = path(here(), "results", "granger", "granger_approach_ofc_mfg.RData"))
...
cat("P(Overall mean > 0):", prob_mu_gt_0, "\n")
#save model
save(mfg_cing_app_model, file = path(here(), "results", "granger",  "granger_approach_mfg_cing.RData"))
```
...
cat("P(Overall mean > 0):", prob_mu_gt_0, "\n")
#save model
save(mfg_cing_app_model, file = path(here(), "results", "granger", "granger_100ms_approach_mfg_cing.RData"))
```
...
total_electrodes <- granger_df %>%
  select(subject, pair_id, region) %>%
  distinct() %>%
  rename(region_pair = region) %>%
  group_by(region_pair) %>%
  summarize(total_electrodes = n())

total_sig_electrodes <- granger_500_sig_df %>%
  select(subject, pair_id, region) %>%
  distinct() %>%
  rename(region_pair = region) %>%
  group_by(region_pair) %>%
  summarize(sig_electrodes = n())

second_regions_leads_electrodes <- granger_500_sig_df %>%
  mutate(case = if_else(times <= 0, "Approach", "Avoid")) %>%
  group_by(case, subject, pair_id) %>%
  mutate(avg_granger = mean(granger)) %>%
  select(-times, -granger, -pval, -pval_fdr, -`Unnamed: 0`, -sig) %>%
  filter(case == "Approach") %>%
  distinct() %>%
  filter(avg_granger >= 0) %>%
  rename(region_pair = region) %>%
  group_by(region_pair) %>%
  summarize(second_region_leads = n())

orig_df <- tibble("Prob Pos." = orig_500, "region_pair" = names(orig_500)) %>%
  mutate(region = region_pair) %>%
  mutate(region_pair = str_to_lower(region_pair))

orig_combined_df <- left_join(orig_df, total_electrodes, by = "region_pair") %>%
  left_join(total_sig_electrodes, by = "region_pair") %>%
  left_join(second_regions_leads_electrodes,  by = "region_pair")

orig_combined_df <- orig_combined_df %>%
  mutate(region = if_else(region == "OFC_MFG", "MFG_OFC", region)) %>%
  mutate(`Prob Pos.` = if_else(region == "MFG_OFC", round(1- `Prob Pos.`, 2), round(`Prob Pos.`, 2))) %>%
  mutate(second_region_leads = if_else(region == "MFG_OFC", sig_electrodes- second_region_leads, second_region_leads)) %>%
  mutate(percent = round(second_region_leads/sig_electrodes, 2)) %>%
  select(region, `Prob Pos.`, total_electrodes, sig_electrodes, second_region_leads, percent) %>%
