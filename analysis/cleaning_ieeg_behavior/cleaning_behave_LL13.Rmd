...
ll13_data <- read_csv(path(here(), './data/ieeg_behave/LL13_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# task vars df #
ll13_all_vars_df <- get_across_task_variables_bci(ll13_data_clean)
...
no_biscuits_trials <- ll13_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
ll13_data_clean %>%
  ...
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
  filter(neural_trial_numeric == 237) %>%
  pivot_longer(cols = c(GhostLocation, UserLocation), values_to = "location", names_to = "unit") %>%
  ggplot(., aes(x = trial_flip, y = location, color = unit)) +
  geom_hline(yintercept = 170, color = "black") +
  geom_hline(yintercept = 10, color = "black") +
...
length(unique(ll13_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/LL13_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/LL13_bad_trials.csv'))

create_last_away_events(ll13_data_clean, 'LL13_last_away_events.csv')
create_first_dot_events(ll13_data_clean, 'LL13_first_dot_events.csv')
create_first_move_events(ll13_data_clean, 'LL13_first_move_events.csv')
create_died_events(ll13_data_clean, 'LL13_died_events.csv')
create_attack_events(ll13_data_clean, 'LL13_attack_events.csv')
