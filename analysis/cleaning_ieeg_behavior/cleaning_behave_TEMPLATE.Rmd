...
```{r XXSUBJECTXX-data-load}

XXSUBJECTXX_data <- read_csv(path(here(), './data/ieeg_behave/XXSUBJECTXX_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
```{r XXSUBJECTXX-data-prep}

# clean Biscuits #
XXSUBJECTXX_data_clean <- clean_bci_data(XXSUBJECTXX_data, 512)

# create distance df #
XXSUBJECTXX_data_distance <- create_distance_df_bci(XXSUBJECTXX_data_clean)

# task vars df #
XXSUBJECTXX_all_vars_df <- get_across_task_variables_bci(XXSUBJECTXX_data_clean)
...
no_biscuits_trials <- XXSUBJECTXX_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
long_trials <- XXSUBJECTXX_all_vars_df %>%
...
length(unique(XXSUBJECTXX_data_clean$trial_numeric))
...
write_csv(bad_df, path(here(), './data/ieeg_behave/XXSUBJECTXX_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/XXSUBJECTXX_bad_trials.csv'))

create_last_away_events(XXSUBJECTXX_data_clean, 'XXSUBJECTXX_last_away_events.csv')
create_first_dot_events(XXSUBJECTXX_data_clean, 'XXSUBJECTXX_first_dot_events.csv')
create_first_move_events(XXSUBJECTXX_data_clean, 'XXSUBJECTXX_first_move_events.csv')
create_died_events(XXSUBJECTXX_data_clean, 'XXSUBJECTXX_died_events.csv')
create_attack_events(XXSUBJECTXX_data_clean, 'XXSUBJECTXX_attack_events.csv')
...
trial_times <- XXSUBJECTXX_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
